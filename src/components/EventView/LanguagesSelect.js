    ...
    return(
      <div style={styles.container} onClick={this._toggleDropdown} ref={node => { this._containerNode = node; }}>
        ...
        <input
            ...
            type='text'
            value={selectedItem
              ? selectedItem.length >= 2 && selectedItem.length <= 3
                ? countries.getName(selectedItem, localizations.getLanguage())
                : selectedItem
              : ''}
            readOnly={true}
            ...
            <div style={label ? styles.dropdownWithLabel : styles.dropdown}>
                <ul style={styles.list}>
                {
                    list.map((item, index) =>
                        ...
                        >
                            {countries.getName(item.code, localizations.getLanguage())}
                        </li>
                    )
                }
                </ul>
