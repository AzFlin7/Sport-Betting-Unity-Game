...
const SummoningInvitedListTemp = createRefetchContainer(Radium(SummoningInvitedList), {
    viewer: graphql`
        fragment SummoningInvitedList_viewer on Viewer @argumentDefinitions(
...
graphql`
    query SummoningInvitedListRefetchQuery($queryCircle: Boolean!, $circleId: ID) {
        viewer {
            ...SummoningInvitedList_viewer @arguments(queryCircle: $queryCircle, circleId: $circleId)
        }
    }
`
)
    ...
    render() {
      return (
        <QueryRenderer
          ...
          query={graphql`
            query SummoningInvitedListQuery {
              viewer {
