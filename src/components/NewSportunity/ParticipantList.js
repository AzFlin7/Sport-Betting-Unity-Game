...
class ParticipantList extends PureComponent {
        ...
        if (this.state.circleClicked) {
            ...
            () => {
                setTimeout(() => {// Needed to wait for Relay to re-fetch data in this.props.viewer
                    if (this.props.viewer.participantListUserExists || isEmail) {
                        if (pseudo) {
    ...
    render() {
        ...
        const { list, invitedCircles, onRemoveItem, onRemoveInvitedCircle, onChangeCirclePrice, onChangeCircleAutoParticipate, onChangeUserAutoParticipate, viewer, renderStepActions } = this.props;
...
export default createRefetchContainer(Radium(withAlert(ParticipantList)), {
        viewer: graphql`
            fragment ParticipantList_viewer on Viewer @argumentDefinitions (
                ...
                participantListUserExists: userExists (pseudo: $pseudo, email: $email)
    ...
    graphql`query ParticipantListRefetchQuery (
        $pseudo: String,
        $email: String,
        ...
        viewer {
            ...ParticipantList_viewer @arguments (
                pseudo: $pseudo,
                email: $email,
