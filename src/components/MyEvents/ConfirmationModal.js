                    ...
                    >
                        <div style={styles.modalContent} ref={node => { this._containerNode = node; }}>
                            ...
                            <span style={styles.confirm}>
                              {localizations.selectingEvent_confirmModal_msg1}
                              <b style={{fontWeight: 'bold'}}>{localizations.formatString(localizations.selectingEvent_nbSelected, this.props.nbSelected)}</b>
                              {localizations.selectingEvent_confirmModal_msg2}<br/>
                              <i style={{fontStyle: 'italic'}}>{localizations.selectingEvent_confirmModal_msg3}</i>
                            </span>
                            <div style={styles.buttonRow}>
                                ...
                                {!this.state.isLoading && 
                                    <button onClick={() => {this.setState({isLoading: true}); this.props.onConfirm(); this._closeModal()}} style={styles.greenButton}>{this.props.confirmLabel}</button>
                                }
                            </div>
                        </div>
...
export function confirmModal(properties) {
    document.body.children[0].classList.add('react-confirm-alert-blur');
    const divTarget = document.createElement('div');
    divTarget.id = 'react-confirm-alert';
    document.body.appendChild(divTarget);
    render(<ConfirmationModal {...properties} isOpen={true} />, divTarget);
}