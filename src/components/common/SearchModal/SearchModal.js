...
const UnStyledCircleItem = ({classes, circle, onItemTap, onCirleMemberItemTap, state, onCheckboxChange, onSelectAllChange, allowToSeeCircleDetails, displayPriceField, handleChangePrice, price_for_circles, currency}) => {
  ...
  return (
    <div key={circle.id}>
      <ListItem style={styles.listItemWrapper} onClick={onItemTap(circle)}>
        ...
        <ListItemText inset primary={circle.name} secondary={circle.owner.pseudo}/>
...
const SearchBar = props => {
  ...
  return (
    <Typography component="div" style={styles.searchBar}>
    ...
    if ((text !== '' && text) || this.props.queryCirclesOnOpen) {
      const refetchVariables = fragmentVariables => {
        let variables = {
          ...fragmentVariables,
          queryMain: true, 
          queryCircles: this.state.activeTab === "Groups",
          circlesFilter:{
            nameCompletion: text, 
            code: text, 
            circleType: this.props.circleTypes || ['MY_CIRCLES', 'CIRCLES_I_AM_IN', 'CHILDREN_CIRCLES', 'PUBLIC_CIRCLES'], 
    ...
    if ((text !== '' && text)) {
      const refetchVariables = fragmentVariables => {
        let variables = {
            ...
            pseudo_autocomplete: this.isValidEmailAddress(text) ? null : text,
            email_autocomplete: this.isValidEmailAddress(text) ? text : null,
            queryCircles: false,
        }
        ...
        >
          <div style={styles.modalContent}>
            ...
            {this.state.activeTab === "People" &&
              <TabContainer>
                <SearchBar
                  placeholder={localizations.searchModule_people_placeholder}
                ...
                <StyledSelectedChip
                  ...
                  selectedMembers={this.state.checked}
                  members={(this.props.viewer && this.props.viewer.users && this.props.viewer.users.edges)
                    ? this.props.viewer.users.edges.map(edge => edge.node)
                    : []
                  }
                  isMembersList={true}
                />
                ...
                : this.props.viewer && this.props.viewer.users && this.props.viewer.users.edges && 
                  (this.props.viewer.users.count > 0 
                    ...
                    : <div style={styles.noResult}>
                        {localizations.searchModule_No_result}
                        {tabs.indexOf("Invite") >= 0 && 
                          <div>
                            {localizations.searchModule_No_result_But_Invite}
                            <SearchBar
                              placeholder="E-mail"
                              onChange={this.handleInviteInputChange}
                              onErase={this.onInviteEraseTap}
                              searchFieldValue={this.state.inviteSearchFieldValue}
                              image="/svg-icons/email.svg"
                            />
                            <InviteList
                              nameFieldValue={this.state.nameFieldValue}
                              onChangeName={this.handleNameChange}
                              onInvite={this.handleInviteTap}
                            />
                          </div>
                        }
                      </div>
            ...
            {this.state.activeTab === "Groups" &&
              <TabContainer>
                <SearchBar
                  placeholder={localizations.searchModule_groups_placeholder}
                  onChange={this.handleCirclesInputChange}
                  onErase={this.handleClearInput}
            ...
            {this.state.activeTab === "PublicGroups" &&
              <TabContainer>
                <SearchBar
                  placeholder={localizations.searchModule_groups_placeholder}
                  onChange={this.handlePublicCirclesInputChange}
                  onErase={this.handleClearInput}
...
const SearchModalTemp = createRefetchContainer(withAlert(Radium(SearchModal)), {
  viewer: graphql`
    fragment SearchModal_viewer on Viewer @argumentDefinitions(
