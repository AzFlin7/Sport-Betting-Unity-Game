...
class SportLevels extends PureComponent {
  ...
  render() {
    ...
    const { label, style, list, disabled, required, placeholder, from, to } = this.props;
    let actualTerm = term ;
      ...
      >
        {
          ...
          ? <span onClick={this._handleRemoveSelection} style={styles.closeCross}>
              <i className="fa fa-times" style={styles.cancelIcon} aria-hidden="true" />
            </span>
